    ...
    <?php foreach ($kabupaten as $key => $value) : ?>
        ...
        <div id="modal<?= $value['id']; ?>" class="fixed hidden top-0 bottom-0 right-0 left-0 bg-slate-900 bg-opacity-50 z-10 p-2 md:p-60">
            <div class="bg-white rounded-md p-4 relative">
                ...
                <p class="text-center font-medium text-lg">Edit Nama Kabupaten</p>
                <form method="post" class="mt-2">

                    <input class="peer w-full p-1 mb-2 outline-none border border-teal-500 rounded-md focus:invalid:border-red-700" type="text" name="kabupatenEdit<?= $value['id']; ?>" id="kabupatenEdit<?= $value['id']; ?>" placeholder="masukkan nama kabupaten..." value="<?= $value['kabupaten']; ?>" required>
    ...
    $(document).ready(() => {
        ...
        hapus = (id) => {
            questionAlert('yakin untuk menghapus data ini ?', () => {
                $.ajax({
                    url: `/kabupaten/deleteData/${id}`,
                    method: 'delete'
                }).done(() => {
                    successAlert('kabupaten berhasil dihapus')
                    $('#table').load('/kabupaten/getData/10/0')
                    $('#page').load('/kabupaten/pageGetData/10')
                }).fail(() => {
                    errorAlert('kabupaten gagal dihapus')
                })
            }, () => {
                infoAlert('kabupaten tidak dihapus')
            })
        }
        ...
        btnEdit = (id) => {
            ...
            formData.append('kabupatenEdit', $('#kabupatenEdit' + id).val())
            $.post('/kabupaten/editData/', formData)
                .done(() => {
                    successAlert('kabupaten berhasil diedit')
                    $('#table').load('/kabupaten/getData/10/0')
                    $('#page').load('/kabupaten/pageGetData/10')
                    $('#modal' + id).addClass('hidden')
                })
                .fail(() => {
                    errorAlert('kabupaten gagal diedit')
                })
        }
