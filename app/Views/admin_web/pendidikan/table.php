    ...
    <?php foreach ($pendidikan as $key => $value) : ?>
        ...
        <div id="modal<?= $value['id']; ?>" class="fixed hidden top-0 bottom-0 right-0 left-0 bg-slate-900 bg-opacity-50 z-10 p-2 md:p-60">
            <div class="bg-white rounded-md p-4 relative">
                ...
                <p class="text-center font-medium text-lg">Edit Nama Pendidikan</p>
                <form method="post" class="mt-2">

                    <input class="peer w-full p-1 mb-2 outline-none border border-teal-500 rounded-md focus:invalid:border-red-700" type="text" name="pendidikanEdit<?= $value['id']; ?>" id="pendidikanEdit<?= $value['id']; ?>" placeholder="masukkan nama pendidikan..." value="<?= $value['pendidikan']; ?>" required>
    ...
    $(document).ready(() => {
        ...
        hapus = (id) => {
            questionAlert('yakin untuk menghapus data ini ?', () => {
                $.ajax({
                    url: `/pendidikan/deleteData/${id}`,
                    method: 'delete'
                }).done(() => {
                    successAlert('pendidikan berhasil dihapus')
                    $('#table').load('/pendidikan/getData/10/0')
                    $('#page').load('/pendidikan/pageGetData/10')
                }).fail(() => {
                    errorAlert('pendidikan gagal dihapus')
                })
            }, () => {
                infoAlert('pendidikan tidak dihapus')
            })
        }
        ...
        btnEdit = (id) => {
            ...
            formData.append('pendidikanEdit', $('#pendidikanEdit' + id).val())
            $.post('/pendidikan/editData/', formData)
                .done(() => {
                    successAlert('pendidikan berhasil diedit')
                    $('#table').load('/pendidikan/getData/10/0')
                    $('#page').load('/pendidikan/pageGetData/10')
                    $('#modal' + id).addClass('hidden')
                })
                .fail(() => {
                    errorAlert('pendidikan gagal diedit')
                })
        }
