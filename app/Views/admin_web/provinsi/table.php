    ...
    <?php foreach ($provinsi as $key => $value) : ?>
        ...
        <div id="modal<?= $value['id']; ?>" class="fixed hidden top-0 bottom-0 right-0 left-0 bg-slate-900 bg-opacity-50 z-10 p-2 md:p-60">
            <div class="bg-white rounded-md p-4 relative">
                ...
                <p class="text-center font-medium text-lg">Edit Nama Provinsi</p>
                <form method="post" class="mt-2">

                    <input class="peer w-full p-1 mb-2 outline-none border border-teal-500 rounded-md focus:invalid:border-red-700" type="text" name="provinsiEdit<?= $value['id']; ?>" id="provinsiEdit<?= $value['id']; ?>" placeholder="masukkan nama provinsi..." value="<?= $value['provinsi']; ?>" required>
    ...
    $(document).ready(() => {
        ...
        hapus = (id) => {
            questionAlert('yakin untuk menghapus data ini ?', () => {
                $.ajax({
                    url: `/provinsi/deleteData/${id}`,
                    method: 'delete'
                }).done(() => {
                    successAlert('provinsi berhasil dihapus')
                    $('#table').load('/provinsi/getData/10/0')
                    $('#page').load('/provinsi/pageGetData/10')
                }).fail(() => {
                    errorAlert('provinsi gagal dihapus')
                })
            }, () => {
                infoAlert('provinsi tidak dihapus')
            })
        }
        ...
        btnEdit = (id) => {
            ...
            formData.append('provinsiEdit', $('#provinsiEdit' + id).val())
            $.post('/provinsi/editData/', formData)
                .done(() => {
                    successAlert('provinsi berhasil diedit')
                    $('#table').load('/provinsi/getData/10/0')
                    $('#page').load('/provinsi/pageGetData/10')
                    $('#modal' + id).addClass('hidden')
                })
                .fail(() => {
                    errorAlert('provinsi gagal diedit')
                })
        }
